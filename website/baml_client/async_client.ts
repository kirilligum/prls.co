...
export class BamlAsyncClient {
  ...
  async CheckRelevance(
      blog_content: string,chat_history_json_string: string,
    ...
    try {
      ...
      const raw = await this.runtime.callFunction(
        "CheckRelevance",
        {
          "blog_content": blog_content,"chat_history_json_string": chat_history_json_string
        },
      ...
      return raw.parsed(false) as SpamCheckResult
    } catch (error) {
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.SpamCheckResult, SpamCheckResult> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      ...
      return new BamlStream<partial_types.SpamCheckResult, SpamCheckResult>(
        raw,
        (a): partial_types.SpamCheckResult => a,
        (a): SpamCheckResult => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
